...
export default class ConfigureHelper {
    ...
    public static makePromise(needToGenerate: ToProcess[], progress: vscode.Progress<{}>, token: vscode.CancellationToken) {
        ...
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < needToGenerate.length; i++) {
                ...
                progress.report({
                    message:
                        `${this.getProgressMessage(doneCount, needToGenerate.length, proc, pid2JobId)}
                             Generating models..`,
                ...
                proc.on('close', (code) => {
                    ...
                    fs.writeFileSync(
                        path.join(vscode.workspace.rootPath!, obj.hash, "vscode.extensionCache"),
                        "blank"
                    )
                ...
                token.onCancellationRequested(() => {
                    ...
                    needToGenerate.map((j) => rimraf.sync(path.join(vscode.workspace.rootPath!, j.hash)))
                    // cached.map((c) => rimraf.sync(path.join(vscode.workspace.rootPath!, c.hash)))
                })
            }
    ...
    public static async separateJobs(list: any): Promise<Separation> {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            const matching = caches.filter((file) => file.path.split("/").includes(hash))
