...
export default class WebviewHelper {
    ...
    panel.webview.onDidReceiveMessage(
      message => {
        switch (message.command) {
          ...
          case "loadNodes":
            request(
              this.serverURL + "/loadNodes/" + message.start,
              { json: true },
              (err: any, res: any) => {
                if (err) {
                  this.handleConnectionError(err);
          ...
          case "longestBranchingVariable":
            request(
              this.serverURL + "/longestBranchingVariable",
              { json: true },
              (err: any, res: any) => {
                if (err) {
                  this.handleConnectionError(err);
  ...
  public static getWebContent(): string {
    ...
    const html = vscode.Uri.file(
      path.join(WebviewHelper.context.extensionPath, "src/webview", "main.html")
    );
    ...
    const scriptPath = vscode.Uri.file(
      path.join(
        WebviewHelper.context.extensionPath,
        "src/webview/ts/dist/",
        "bundle.js"
      )
    );
