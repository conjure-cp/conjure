...
export default class Globals {
    ...
    public static loadDomains(vscodeApi: any) {
        if (State.pretty) {
            ...
            Web.getRequest('prettyDomains/' + State.selectedId + "/" + !$("#expressions").prop("checked") + "/" + State.pathList.join(":"))
                .then(response => {
                    let data = JSON.parse(response);
                    // console.log(data);
            ...
            Web.getRequest('simpleDomains/' + State.selectedId + "/" + !$("#expressions").prop("checked"))
                .then(response => {
                    let data = JSON.parse(response);
                    console.log(data);
    ...
    public static initialize(vscodeApi: any) {
        ...
        Web.getRequest('init//home/tom/SearchTreeVisualisationTests/testData/gears1000/')
            .then(response => {
                let data = JSON.parse(response);
