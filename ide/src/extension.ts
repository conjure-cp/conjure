...
export function activate(context: vscode.ExtensionContext) {
	...
	fp(7000, 8000, '127.0.0.1', 2, (err: any, nimServerPort: number, vscodeServePort: number) => {
		if (err) {
		...
		context.subscriptions.push(
			...
			vscode.commands.registerCommand('conjure.configure', () => {
				...
				vscode.window.showInformationMessage(`http://localhost:${vscodeServePort}`)
				vscode.window.showInformationMessage(`Tree visualiser running at:      `)
			}),
		)
		context.subscriptions.push(
