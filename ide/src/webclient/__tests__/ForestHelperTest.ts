...
import { fetchAncestors } from '../src/modules/ForestHelper'
import {
...
describe('testing ForestHelper', () => {
	...
	beforeEach(async () => {
		...
		let res = await loadAllDiffs([ '', 's' ], [ coreNormal8, coreSacbounds8 ], normalVSSacbounds, 5000)
	...
	describe('testing mergeMaps', () => {
		...
		it('Should merge the maps the ancestors of each tree into their maps big->small', async () => {
			...
			let diff1 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[0].leftTreeId)!
			let diff2 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[1].leftTreeId)!
			let diff3 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[2].leftTreeId)!
			let diff4 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[3].leftTreeId)!
			let diff5 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[4].leftTreeId)!
			...
			expect(diff1.children!.map((x) => x.treeId)).toEqual([ WhichTree.Left, WhichTree.Both ])
			expect(diff2.children!.map((x) => x.id)).toEqual([ 8, 11 ])
			expect(diff2.children!.map((x) => x.treeId)).toEqual([ WhichTree.Left, WhichTree.Left ])
			expect(diff3.children!.map((x) => x.id)).toEqual([ 18, 21 ])
			expect(diff3.children!.map((x) => x.treeId)).toEqual([ WhichTree.Left, WhichTree.Both ])

			expect(diff4.children!.map((x) => x.id)).toEqual([ 22, 24 ])
			expect(diff4.children!.map((x) => x.treeId)).toEqual([ WhichTree.Left, WhichTree.Left ])

			expect(diff5.children!.map((x) => x.id)).toEqual([ 28, 30, 10 ])

			expect(diff5.children!.map((x) => x.treeId)).toEqual([ WhichTree.Left, WhichTree.Left, WhichTree.Right ])
		})
		...
		it('Should merge the maps the ancestors of each tree into their maps small->big', async () => {
			...
			let diff1 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[0].rightTreeId)!
			let diff2 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[1].rightTreeId)!
			let diff3 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[2].rightTreeId)!
			let diff4 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[3].rightTreeId)!
			let diff5 = getDescList(res[0]).find((x) => x.id === normalVSSacbounds[4].rightTreeId)!
			...
			expect(diff1.children!.map((x) => x.treeId)).toEqual([ WhichTree.Both, WhichTree.Right ])
			expect(diff2.children!.map((x) => x.id)).toEqual([ 8, 11 ])
			expect(diff2.children!.map((x) => x.treeId)).toEqual([ WhichTree.Right, WhichTree.Right ])
			expect(diff3.children!.map((x) => x.id)).toEqual([ 7, 18 ])
			expect(diff3.children!.map((x) => x.treeId)).toEqual([ WhichTree.Both, WhichTree.Right ])

			expect(diff4.children!.map((x) => x.id)).toEqual([ 22, 24 ])
			expect(diff4.children!.map((x) => x.treeId)).toEqual([ WhichTree.Right, WhichTree.Right ])
			expect(diff5.children!.map((x) => x.id)).toEqual([ 10, 28, 30 ])
			expect(diff5.children!.map((x) => x.treeId)).toEqual([ WhichTree.Left, WhichTree.Right, WhichTree.Right ])
		})
	})
	...
	describe('test fetch ancestors', () => {
		...
		it('returns the ancestors (including the current node)', () => {
			fetchMock.mockResponseOnce(JSON.stringify([ new Node(4, 'poop', '', 3, 0, true, 0, false) ]))
