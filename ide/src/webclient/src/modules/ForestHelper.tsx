...
export const collapseUnwantedDiffs = (
	...
	diffPoints.forEach((diffPoint) => {
		...
		if (rightMap[diffPoint.rightTreeId] && rightMap[diffPoint.rightTreeId].children) {
		...
		if (
			...
			if (diffPoint.rightTreeId in rightMap && rightMap[diffPoint.rightTreeId].children) {
				rightMap[diffPoint.rightTreeId].children!.forEach((x) => delete rightMap[x.id])
...
export const assignTreeIds = (leftMap: MyMap, rightMap: MyMap, diffPoints: DiffPoint[]) => {
...
export const getDescList = (root: Node) => {
	return d3.hierarchy<Node>(root).descendants().slice(1).map((x) => x.data)
}
...
export const getAncList = (root: Node, startId: number, treeId: WhichTree) => {
	...
	if (treeId === WhichTree.Right) {
		current = nodes.find((x) => x.data.id === startId && x.data.treeId === WhichTree.Right)!
	} else {
		current = nodes.find((x) => x.data.id === startId && x.data.treeId !== WhichTree.Right)!
	}
	...
	return current.ancestors().map((x) => x.data).slice(1).reverse()
}

export const loadDiff = async (paths: string[], maps: MyMap[], diffPoint: DiffPoint, nimServerPort: number) => {
...
export const loadAllDiffs = async (paths: string[], cores: Core[], diffPoints: DiffPoint[], nimServerPort: number) => {
	...
	for (const diffPoint of diffPoints) {
		if (leftMap[diffPoint.leftTreeId]) {
			diffPoint.highlightRight.forEach((nodeId) => {
				if (rightMap[diffPoint.rightTreeId] && rightMap[diffPoint.rightTreeId].children && rightMap[nodeId]) {
...
export const assignNewDescCounts = (merged: MyMap, diffPoints: DiffPoint[]) => {
	...
	let nodeList = [ merged[0] ].concat(getDescList(merged[0])).reverse().filter((x) => x.treeId === WhichTree.Both)

	nodeList.filter((x) => copy.map((y) => y.leftTreeId).includes(x.id)).forEach((x) => (x.childCount = 2))
