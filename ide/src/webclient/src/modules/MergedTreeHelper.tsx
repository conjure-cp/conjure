...
import { getAncList, mergeMaps, getDescList, loadDiff, assignTreeIds } from './ForestHelper'
...
const getDiffPointKids = (
	...
	if (currentTreeId !== WhichTree.Right && leftDiffIds.includes(currentSelected)) {
...
export const goRightMerged = async (
	...
	let diffPointKids = getDiffPointKids(leftMap, rightMap, currentSelected, currentTreeId, diffPoints)
	if (diffPointKids) {
		// console.log(kids)
...
export const goDownMerged = async (
	...
	let kids = getDiffPointKids(leftMap, rightMap, currentSelected, currentTreeId, diffPoints)
...
export const goLeftAtDiffingPoint = async (
	...
	let maps = await loadDiff([ leftPath, rightPath ], [ leftMap, rightMap ], diffPoints[index], nimServerPort)
...
export const reviseGoLeft = (mergedMap: MyMap, currentSelected: number, treeId: WhichTree, diffPoints: DiffPoint[]) => {
...
export const shouldBeRightTree = (
	...
	if (leftMap[nextSelected] && rightMap[nextSelected] && rightMap[nextSelected].treeId !== WhichTree.Both) {
		return true
	}

	// If it aint in the left or both but is in the right then go right
	if (!leftMap[nextSelected] && rightMap[nextSelected] && rightMap[nextSelected].treeId === WhichTree.Right) {
		return true
	}
...
export const goLeftMerged = async (
	...
	let leftDiffIds = diffPoints.map((x) => x.leftTreeId)
	if (leftDiffIds.includes(currentSelected) && currentTreeId !== WhichTree.Right) {
		return {
	...
	let res = await goLeftBoyo(currentSelected, map!, false, false, path, 1, nimServerPort, nextTreeId)
	...
	if (isRightTree) {
		...
		leftMap = res.id2Node
		if (getDescList(leftMap[nextSelected]).filter((x) => leftDiffIds.includes(x.id)).length > 0) {
	...
	if (isRightTree) {
		if (shouldBeRightTree(leftMap, rightMap, nextSelected, isRightTree)) {
			...
			let revision = reviseGoLeft(mergedMap, currentSelected, nextTreeId, diffPoints)
		...
		if (ancestors && !ancestors.concat(leftMap[nextSelected]).find((x) => x.treeId === currentTreeId)) {
			nextTreeId = WhichTree.Both
		}
	}
