...
import { HierarchyPointLink, HierarchyPointNode, Selection, precisionFixed } from 'd3'
...
export default class TreeVis extends React.Component<Props, State> {
	...
	constructor(props: Props) {
		...
		this.zoom = d3
			.zoom<any, any>()
			.on('zoom', function() {
				d3.select(`#${props.identifier}thegroup`).attr('transform', d3.event.transform)
			})
	...
	focusNode(node: HierarchyPointNode<Node>) {
		this.zoom.translateTo(
			d3.select(`#${this.props.identifier} svg`).transition().duration(this.props.duration),
			node.x,
			node.y,
		)
	}
	...
	updateCircles(selector: any) {
		...
		circle
			...
			.attr('r', (d: HierarchyPointNode<Node>) =>
				Node.getRadius(d, this.props.linScale, this.props.minsize, this.props.diffParentId),
			)

		circle.style('transition', `stroke ${this.props.duration}ms, fill ${this.props.duration}ms`)

		circle.classed('different', (d: HierarchyPointNode<Node>) => {
			return d.data.id === this.props.diffParentId
		})

		circle.classed('selected', (d: HierarchyPointNode<Node>) => d.data.id === this.props.selected)

		circle.classed('hasOthers', (d: HierarchyPointNode<Node>) => this.hasHiddenChildren(d))
		...
		circle.classed('explored', (d: HierarchyPointNode<Node>) => this.hasOnlyExploredChildren(d))

		circle.classed('solution', (d: HierarchyPointNode<Node>) => d.data.isSolution)
	}

	getPrevPos = (d: HierarchyPointNode<Node>): { x: number; y: number } => {
		return this.state.oldPos[d.data.id] ? this.state.oldPos[d.data.id] : { x: -1, y: -1 }
	}

	getParentNode = (d: HierarchyPointNode<Node>, nodeList: HierarchyPointNode<Node>[]) => {
		return nodeList.filter((k) => k.data.id === d.parent!.data.id)[0]
	}

	getNode = (d: HierarchyPointNode<Node>, nodeList: HierarchyPointNode<Node>[]) => {
	...
	drawTree() {
		...
		const sorted = hierarchy.descendants().sort((a, b) => b.data.label.length - a.data.label.length)

		const maxRadius = this.props.linScale(this.props.rootNode.descCount)

		const maxWidth = this.props.showLabels ? sorted[0].data.label.length * 10 : maxRadius * 1.5
		...
		let nodeEnter = node.enter().append('g').attr('class', 'node').on('click', (d: HierarchyPointNode<Node>) => {
		...
		nodeEnter
			.attr('transform', (d) => {
				let entering = d.parent
					? `translate(${this.getPrevPos(d.parent).x},${this.getPrevPos(d.parent).y})`
					: ''
		...
		nodeUpdate.transition().duration(this.props.duration).attr('transform', (d: HierarchyPointNode<Node>) => {
		...
		nodeExit.selectAll('text').transition().duration(this.props.duration).style('fill-opacity', 1e-6)

		nodeExit.select('circle').transition().duration(this.props.duration).attr('r', 0)
		...
		const enterLink = link
			...
			.classed('link', true)
			.classed('red', (d) => {
				return !this.props.solAncestorIds.includes(d.target.data.id) || !this.props.solveable
			})
			.classed(
				'different',
				(d) =>
