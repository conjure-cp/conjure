...
class Forest extends React.Component<Props, State> {
	...
	componentDidUpdate = async (prevProps: Props) => {
		...
		let json = await fetch(`http://localhost:${this.props.nimServerPort}/diff`, {
			method: 'post',
			headers: headers,
					...
					<Fragment>
						...
						<DiffSettings
							...
							nextDiffHandler={() => {
								...
								if (this.state.currentDiffIndex + 1 > this.state.diffPoints.length - 1) {
									this.setState({
								...
								this.state.diffReady && (
									...
									<div style={{ width: '100%' }}>
										<MergedTreeContainer
											...
											diffPoints={this.state.diffPoints}
											nimServerPort={this.props.nimServerPort}
											leftSolAncestorIds={this.props.trees[0].core.solAncestorIds}
											rightSolAncestorIds={this.props.trees[1].core.solAncestorIds}
											duration={this.state.duration}
											interval={this.state.interval}
											showLabels={this.state.showLabels}
										/>
