...
function perModelperParam {
    ...
    # echo "minion -instancestats $MODEL-$PARAM.eprime-minion > $MODEL-$PARAM.eprime-minion-stats"
    time minion -instancestats                                                   \
        $MODEL-$PARAM.eprime-minion                                         \
        > $MODEL-$PARAM.eprime-minion-stats
...
function perParam {
    echo "[generatePlotData] for $1"
    cd $OUT_DIR
    ${CONJURE_REPO}/experiments/learning_cp2013/generatePlotData $1 \
        > ${CONJURE_REPO}/experiments/learning_cp2013/plots/${SPEC}-$1.js
}
export -f perParam;
