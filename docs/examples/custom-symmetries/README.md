...
The constraint fixes `f(1:E) = 1:E`. Only the identity and the swap of 2 and 3 are supplied: the full symmetric group would not preserve this problem. Eager keeps one representative from each of the six orbits. Change the operator name to compare Delayed with Eager using the same parameter file.
