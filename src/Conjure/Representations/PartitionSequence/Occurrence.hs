...
module Conjure.Representations.PartitionSequence.Occurrence ( partitionSequenceOccurrence ) where
...
-- | works for "partitionSequence from A", where A can be used as an index of a matrix
...
partitionSequenceOccurrence :: forall m . (MonadFail m, NameGen m, EnumerateDomain m) => Representation m
partitionSequenceOccurrence = Representation chck downD structuralCons downC up

    where

        chck :: TypeOf_ReprCheck m
        chck f (DomainPartitionSequence _ attrs innerDomain)
            | domainCanIndexMatrix innerDomain
            = map (DomainPartitionSequence PartitionSequence_Occurrence attrs) <$> f innerDomain
        chck _ _ = return []
        ...
        downD (name, domain@(DomainPartitionSequence PartitionSequence_Occurrence attrs innerDomain))
        ...
        structuralCons _ downX1 (DomainPartitionSequence _ attrs innerDomain)
                | domainCanIndexMatrix innerDomain = do
            maxNumParts <- getMaxNumParts attrs innerDomain
            let
            ...
            return $ \ inpPartitionSequence -> do
                [numPartsVar, whichPart, partSizesVar, firstIndexVar] <- downX1 inpPartitionSequence
                concat <$> sequence
        ...
        downC ( name
              , inDom@(DomainPartitionSequence PartitionSequence_Occurrence attrs innerDomain)
              , inConstant@(ConstantAbstract (AbsLitPartitionSequence vals))
              ) = do
        ...
        up :: TypeOf_Up m
        up ctxt (name, domain@(DomainPartitionSequence PartitionSequence_Occurrence _ innerDomain)) =
