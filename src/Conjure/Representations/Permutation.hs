...
permutationAsFunction dispatch = Representation chck downD structuralCons downC up
 where
   ...
   outName name domain = mkOutName (Just "PermutationFunction") name domain
   ...
   outDomain (DomainPermutation Permutation_AsFunction _ innerDomain) = do
       ...
       return (DomainFunction (Function_1D)
                 (FunctionAttr (SizeAttr_Size s) PartialityAttr_Total JectivityAttr_Bijective) innerDomain innerDomain)
   outDomain domain = na $ vcat [ "{outDomain} PermutationAsFunction"
   ...
   downD (name, domain@(DomainPermutation Permutation_AsFunction _ innerDomain))
       ...
       return $ Just
           ...
           (FunctionAttr (SizeAttr_Size m) PartialityAttr_Total JectivityAttr_Bijective) innerDomain innerDomain
             )
           ] 
     ...
     = return $ \inpFun -> do
         ...
         case refs of
           [fun] -> do
             ...
             concat <$> sequence [ innerStructuralConsGen fun
                                 , return $ mkSizeCons s [essence|
                          sum([ toInt(&i != image(&fun, &i)) | &iPat : &innerDom ])
                                                |]
                                 ]
           _ -> na $ vcat [ "{structuralCons} PermutationAsFunction"
                          , pretty inDom
                          ]
   structuralCons _ _ inDom =
        na $ vcat [ "{structuralCons} PermutationAsFunction"
                  , pretty inDom
                  ] 
   ...
   up :: TypeOf_Up m
   up ctxt ( name
           , domain@(DomainPermutation Permutation_AsFunction{} _ _)) = do 
                case ( lookup (outName domain name) ctxt) of
                  ...
                  _ -> fail $ vcat $
                          ...
                          ("Bindings in context:" : prettyContext ctxt)
   up _ (name, domain) = na $ vcat [ "{up} PermutationAsFunction" 
                                   , "name:" <+> pretty name
                                   , "domain:" <+> pretty domain
                                   ]
