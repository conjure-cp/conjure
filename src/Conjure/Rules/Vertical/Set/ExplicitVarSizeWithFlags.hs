...
rule_frameUpdate_propagator = "set-frameUpdate{ExplicitVarSizeWithFlags}" `namedRule` theRule where
    theRule p = do
...
rule_frameUpdate_decomposition = "set-frameUpdate{ExplicitVarSizeWithFlags}" `namedRule` theRule where
    theRule p = do
