...
mkOp op xs = case op of
  ...
  FunctionOp lex -> case lex of
            ...
            L_catchUndef   -> inject $ MkOpCatchUndef   $ OpCatchUndef   (arg xs 0 "catchUndef")
                                                                         (arg xs 1 "catchUndef")
            L_quickPermutationOrder -> inject $ MkOpQuickPermutationOrder $ OpQuickPermutationOrder (arg xs 0 "quickPermutationOrder" |> listOut |> fromMaybe (bug "")) (arg xs 1 "quickPermutationOrder")
            L_completePermutationOrder -> inject $ MkOpCompletePermutationOrder $ OpCompletePermutationOrder (arg xs 0 "completePermutationOrder" |> listOut |> fromMaybe (bug "")) (arg xs 1 "completePermutationOrder")
            L_applySymmetries -> inject $ MkOpApplySymmetries $ OpApplySymmetries False (arg xs 0 "applySymmetries") (arg xs 1 "applySymmetries")
            L_applySymmetriesQuick -> inject $ MkOpApplySymmetries $ OpApplySymmetries True (arg xs 0 "applySymmetriesQuick") (arg xs 1 "applySymmetriesQuick")
