...
mkOp op xs = case op of
  ...
  FunctionOp lex -> case lex of
            ...
            L_catchUndef   -> inject $ MkOpCatchUndef   $ OpCatchUndef   (arg xs 0 "catchUndef")
                                                                         (arg xs 1 "catchUndef")
            L_permutationOrderDelayed -> inject $ MkOpPermutationOrderDelayed $ OpPermutationOrderDelayed (arg xs 0 "permutationOrderDelayed" |> listOut |> fromMaybe (bug "")) (arg xs 1 "permutationOrderDelayed")
            L_permutationOrderEager -> inject $ MkOpPermutationOrderEager $ OpPermutationOrderEager (arg xs 0 "permutationOrderEager" |> listOut |> fromMaybe (bug "")) (arg xs 1 "permutationOrderEager")
            L_applySymmetriesEager -> inject $ MkOpApplySymmetries $ OpApplySymmetries False (arg xs 0 "applySymmetriesEager") (arg xs 1 "applySymmetriesEager")
            L_applySymmetriesDelayed -> inject $ MkOpApplySymmetries $ OpApplySymmetries True (arg xs 0 "applySymmetriesDelayed") (arg xs 1 "applySymmetriesDelayed")
