...
lexemeWidth :: Lexeme -> Integer
lexemeWidth L_Tab = 4
lexemeWidth (LIntLiteral i) = genericLength (show i)
lexemeWidth (LIdentifier i) = toInteger $ T.length i
lexemeWidth (LComment    i) = toInteger $ T.length i
lexemeWidth l = case lookup l (map swap lexemes) of
    Nothing -> 0
    Just t  -> toInteger $ T.length t
