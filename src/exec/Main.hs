...
data Strategy
    ...
    | Strategy_interactiveFixedQs
    | Strategy_interactiveFixedQsAutoA
    | Strategy_pickFirst
    | Strategy_allFixedQs
    deriving (Eq, Ord, Show, Data, Typeable)

instance Default Strategy where def = Strategy_interactive


ui :: UI
ui = modes
     [ Modelling
         { strategy         = def   &= opt (def :: Strategy)
                                    &= help "Uses the 'interactive' strategy by default."
         , essence          = def   &= typFile
