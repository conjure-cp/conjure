...
typedef struct Buf { char * start; size_t size, count; } Buf;
...
typedef struct Stats {
  ...
  struct {
    int64_t decs, pure, calls, sat, uns, lim, levels;
    struct { int64_t confs, decs, props; } child;
  } prune;
  int64_t prgss, irrprgss, enlwchs, pshwchs, dense, sparse;
  int64_t confs, decisions, hdecs, qdecs, randecs, randphases;
...
#define TIMESLEVEL0     lookahead
    ...
    double showscoredist, showfeatures, prune;                  // level 1
...
typedef struct PruneLimit { int64_t lim, inc; } PruneLimit;

typedef struct Limits {
  int lkhdpen;
  int64_t randec, randphase, dfg;
  struct {
    int64_t visits;
  ...
  struct { int64_t decisions, inc; } prune;
} Limits;
...
static int lgladdcls (LGL * lgl, int red, int origlue, int force) {
  ...
  if (size == 2) {
    ...
    // if (red) 
    {
      if (force && lglval (lgl, lit) < 0) lglf2rce (lgl, other, lit, red);
      if (force && lglval (lgl, other) < 0) lglf2rce (lgl, lit, other, red);
    }
    if (!red && lgl->dense) {
      assert (!red);
      lglincocc (lgl, lit);
...
static void lglbumpreasonlit (LGL * lgl, int lit) {
  AVar * av = lglavar (lgl, lit);
  if (av->mark) return;
  if (!lglevel (lgl, lit)) return;
  av->mark = 1;
  lglpushstk (lgl, &lgl->seen, lit);
  LOG (2, "also bumping reason literal %d", lit);
}

static void lglbumpreason (LGL * lgl, int lit) {
  int * rsn, tag, other, found;
  const int * p;
  rsn = lglrsn (lgl, lit);
  tag = rsn[0] & MASKCS;
  if (tag == DECISION) return;
  if (tag == BINCS || tag == TRNCS) {
    lglbumpreasonlit (lgl, rsn[0] >> RMSHFT);
    if (tag == TRNCS) lglbumpreasonlit (lgl, rsn[1]);
  } else {
    assert (tag == LRGCS);
    p = lglidx2lits (lgl, (rsn[0] & REDCS), rsn[1]);
    found = 0;
    while ((other = *p++)) {
      if (other == -lit) found++;
      else lglbumpreasonlit (lgl, other);
    }
    assert (found == 1);
  }
}

static void lglbumpreasonlits (LGL * lgl) {
  int n = lglcntstk (&lgl->seen);
  const int * p = lgl->clause.start;
  while (*p) lglbumpreason (lgl, *p++);
  for (p = lgl->seen.start + n; p < lgl->seen.top; p++) {
    AVar * av = lglavar (lgl, *p);
    assert (av->mark);
    av->mark = 0;
  }
  LOG (2, "also bumping %d literals", (int)(lglcntstk (&lgl->seen) - n));
}
...
static void lglbumpseenlits (LGL * lgl) {
  ...
  if (lgl->opts->bumpreasonlits.val) lglbumpreasonlits (lgl);
...
static void lglrndcls (LGL * lgl, int len, int prune) {
  int64_t delta;
  if (len == 2) {
    lglrep (lgl, 1 + !prune, prune ? 'B' : 'b');
    lgl->stats->bins++;
  }
  if (len == 3) {
    lglrep (lgl, 1 + !prune, prune ? 'T' : 't');
    lgl->stats->trns++;
  }
  delta = lgl->stats->confs - lgl->confatlastit;
  LOG (1, "iteration delta %lld", (LGLL) delta);
  lglupdatemacd (lgl, &lgl->stats->its.avg, delta);
  if (len == 1) {
    int delay = lgl->opts->simpitdelay.val;
    lgl->confatlastit = lgl->stats->confs;
    lgl->stats->its.count++;
    lglrep (lgl, 1, prune ? 'I' : 'i');
...
DONE:
  ...
  if (!lgl->simp) lglrndcls (lgl, len, 0);
  lglclnana (lgl);
  lglstop (lgl);
...
static int lgldecidephase (LGL * lgl, int lit) {
  ...
  if (lgl->opts->randphase.val &&
      lgl->limits->randphase <= lgl->stats->decisions) {
    LOG (1, "flipping phase due to random phases enabled");
    lgl->limits->randphase = lgl->stats->decisions;
    lgl->limits->randphase += lgl->opts->randphaseint.val/2;
    lgl->limits->randphase += lglrand (lgl) % lgl->opts->randphaseint.val;
...
static int lgldecide (LGL * lgl) {
  ...
  } else if (!(lit = lglprunedecide (lgl))) {
    if (lgl->stats->stability.level > 0 &&
  ...
  if (lit && lit != INT_MIN) lgldassume (lgl, lit);
  return 1;
}
...
static int lglflushed (LGL * lgl) {
  assert (lgl->flushed <= lglcntstk (&lgl->trail));
  return lgl->flushed == lglcntstk (&lgl->trail);
}
...
static int lglgaussextractxor (LGL * lgl, const int * c) {
  ...
  if (lglgaussextractexactly1 (lgl, c)) res = 1;
    ...
    int old = lglcntstk (&lgl->gauss->xors);
...
static int lglpruning (LGL * lgl) {
  if (lgl->mt) return 0;
  if (!lgl->unassigned) return 0;
  if (!lgl->opts->prune.val) return 0;
  if (lgl->opts->drupligcheck.val) return 0;
  if (!lglmtstk (&lgl->assume)) return 0;
  assert (!lgl->alevel);
  if (!lgl->level) return 0;
  if (lgl->level > lgl->opts->prunelevel.val) return 0;
  return lgl->limits->prune.decisions <= lgl->stats->decisions;
}

static void lglictrav (LGL * lgl,
  int internal, int redtoo,
  void * state, void (*trav)(void *, int red, int lit));
...
typedef struct PruneState PruneState;

static void lglpruneaddcls (PruneState * state, int red) {
  int sat = 0, nfalse = 0, ntrue = 0, nunassigned = 0, size = 0;
  const int * p;
  int * q;
#ifndef NLGLOG
  LGL * lgl = state->parent;
  int origsize = lglcntstk (&state->clause);
  const char * redstr = red ? "redundant" : "irredundant";
  lglpushstk (state->parent, &state->clause, 0);
  lglpopstk (&state->clause);
  LOGCLS (4, state->clause.start,
    "trying to add parent %s size %d clause",
    redstr, origsize);
#endif
  for (p = state->clause.start; !sat && p < state->clause.top; p++) {
    const int plit = *p;
    int tmp = lglifixed (state->parent, plit);
    if (tmp < 0) {
      LOG (3, "ignoring root-level falsified literal %d", plit);
    } else if (tmp > 0) {
      LOG (3, "found root-level satisfied literal %d", plit);
      sat = 1;
    } else {
      tmp = lglval (state->parent, plit);
      if (tmp < 0) nfalse++;
      else if (tmp > 0) ntrue++;
      else nunassigned++;
      size++;
  ...
  if (sat) {
    LOGCLS (3, state->clause.start,
      "ignoring satisfied parent %s size %d clause",
      redstr, origsize);
  } else if (size == nunassigned) {
    LOGCLS (3, state->clause.start,
      "ignoring completely unassigned parent %s size %d clause",
      redstr, origsize);
  } else if (!ntrue) {
    LOGCLS (3, state->clause.start,
      "ignoring parent %s size %d clause "
      "without true but %d false literals",
      redstr, origsize, nfalse);
  } else {
    assert (size > 0);
    assert (ntrue > 0);
    LOGCLS (3, state->clause.start,
      "copying parent %s original size %d clause "
      "with %d true and %d false literals and %d skipped",
      redstr, origsize, ntrue, nfalse, nunassigned);
    q = state->clause.start;
    for (p = q; p < state->clause.top; p++) {
      int plit = *p, tmp = lglval (state->parent, plit);
      if (tmp && !lglifixed (state->parent, plit)) {
	int pidx = abs (plit), cidx = state->map[pidx], clit;
	if (!cidx) {
	  cidx = ++state->nvars;
	  LOG (3,
	    "mapping parent internal variable %d "
	    "to child external variable %d",
	    pidx, cidx);
	  state->map[pidx] = cidx;
	}
	clit = plit < 0 ? -cidx : cidx;
	*q++ = clit;
      }
    }
    state->clause.top = q;
#ifndef NLGLOG
    size = lglcntstk (&state->clause);
    lglpushstk (state->parent, &state->clause, 0);
    lglpopstk (&state->clause);
    LOGCLS (4, state->clause.start,
      "add child %s size %d external clause",
      redstr, size);
#endif
    if (red) state->red++; else state->irr++;
    for (p = state->clause.start; p < state->clause.top; p++)
      lgladd (state->child, *p);
    lgladd (state->child, 0);
  }
  lglclnstk (&state->clause);
}

static int lglprunered (LGL * lgl) {
  if (lgl->opts->block.val) return 1;
  if (lgl->opts->prunered.val) return 1;
  return 0;
}

static void lglpruneadd (void * ptr, int red, int lit) {
  PruneState * state = ptr;
  if (red && !lglprunered (state->parent)) return;
  if (lit) lglpushstk (state->parent, &state->clause, lit);
  else lglpruneaddcls (state, red);
}

static void lglpruneana (PruneState * state) {
  LGL * parent = state->parent;
#ifndef NLGLDRUPLIG
  LGL * child = state->child;
  int first = 0;
#endif
#ifndef NLGLOG
  LGL * lgl = parent;
  ...
  int len;
  Ctr * c;
  assert (parent->level - parent->alevel > 0);
  assert (lglmtstk (&parent->seen));
  assert (lglmtstk (&parent->clause));
  assert (!parent->opts->drupligcheck.val);
  LOG (2, "analyzing and pruning level %d clause", parent->level);
  for (c = parent->control.start + 1; c < parent->control.top; c++) {
    int decision = c->decision;
    lglpushstk (parent, &parent->clause, -decision);
    lglpushstk (parent, &parent->seen, decision);
#ifndef NLGLDRUPLIG
    if (lgldruplig (parent)) {
      if (!first) {
	int pidx = abs (decision);
	int cidx = state->map[pidx];
	if (cidx)  {
	  int clit = (decision < 0 ? -cidx : cidx);
	  int tmp = lglderef (child, clit);
	  assert (tmp);
	  if (tmp < 0) {
	    first = -decision;
	    LOG (3, "first (parent internal) literal %d", first);
	  }
	}
      }
      druplig_add_literal (parent->druplig, lglexport (parent, -decision));
    }
#endif
  }
#ifndef NLGLDRUPLIG
  if (lgldruplig (parent)) {
    int found = 0, pidx;
    assert (first);
    druplig_add_literal (parent->druplig, lglexport (parent, first));
    for (pidx = 2; pidx < parent->nvars; pidx++) {
      int cidx = state->map[pidx], tmp, lit;
      if (!cidx) continue;
      tmp = lglderef (child, cidx);
      assert (tmp);
      lit = (tmp < 0) ? -pidx : pidx;
      assert (first != -lit);
      if (first == lit) found++;
      else druplig_add_literal (parent->druplig, lglexport (parent, lit));
    }
    lgldrupligreallyadd (parent, REDCS);
    assert (found == 1), (void) found;
  }
#endif
  lglbumpseenlits (parent);
  len = parent->level;
  assert (len == lglcntstk (&parent->clause));
  parent->stats->lits.nonmin += len;
  parent->stats->clauses.glue += len-1;
  parent->stats->clauses.realglue += len-1;
  parent->stats->lits.learned += len;
  parent->stats->clauses.learned++;
  parent->stats->mincls.size++;
  parent->stats->decflipped++;
  parent->stats->confs++;
  lglbacktrack (parent, len-1);
  lglpushstk (parent, &parent->clause, 0);
  (void) lgladdcls (parent, (lglprunered (parent) ? REDCS : 0), len-1, 1);
  lglrndcls (parent, len, 1);
  lglclnstk (&parent->clause);
  lglclnstk (&parent->seen);
}

static void lglsetpruneprefix (LGL * child, LGL * parent, int level) {
  int len = strlen (parent->prefix);
  int allocated = len + 80;
  char * prefix = lglnew (child, allocated);
  char buffer[80];
  sprintf (buffer,
    "[prune-%lld-%lld-%d] .. ",
    (LGLL) parent->stats->prune.decs,
    (LGLL) parent->stats->prune.calls,
    level);
  strcpy (prefix, parent->prefix);
  if (len && child->prefix[len-1] != ' ') strcat (prefix, " ");
  strcat (prefix, buffer);
  lgldelstr (child, child->prefix);
  child->prefix = lglstrdup (child, prefix);
  lgldel (child, prefix, allocated);
}

static void lglupdprunelim (LGL * lgl, int success) {
  if (success) {
    lgl->limits->prune.inc /= 2;
    if (lgl->limits->prune.inc < lgl->opts->prunemin.val)
      lgl->limits->prune.inc = lgl->opts->prunemin.val;
  } else {
    int64_t inc = lgl->limits->prune.inc/10;
    if (!inc) inc = 1;
    lgl->limits->prune.inc += inc;
    if (lgl->limits->prune.inc > lgl->opts->prunemax.val)
      lgl->limits->prune.inc = lgl->opts->prunemax.val;
  }
  lgl->limits->prune.decisions =
    lgl->stats->decisions + lgl->limits->prune.inc;
  lglprt (lgl, 2,
    "[prune-%lld-%lld] %s new decision increment %lld limit %lld",
    (LGLL) lgl->stats->prune.decs,
    (LGLL) lgl->stats->prune.calls,
    success ? "successful" : "unsuccessful",
    (LGLL) lgl->limits->prune.inc, (LGLL) lgl->limits->prune.decisions);
}
...
static int lglprunedecidefirst (LGL * lgl) {
  int valit, valother, valother2, decision, pos, neg, level, glue, sat;
  int idx, sign, lit, blit, tag, red, other, other2, size, pure, tmp;
  const int skipred = !lglprunered (lgl);
  const int * p, * w, * eow, * c;
  int * bincount, * count;
  long best, score;
  HTS * hts;
  Stk * s;
  NEW (count, 2*lgl->nvars);
  NEW (bincount, 2*lgl->nvars);
  bincount += lgl->nvars;
  count += lgl->nvars;
  level = lgl->level - lgl->alevel;
  assert (!level);
  for (idx = 2; idx < lgl->nvars; idx++) {
    for (sign = -1; sign <= 1; sign += 2) {
      lit = sign * idx;
      valit = lglval (lgl, lit);
      if (valit > 0) continue;
      hts = lglhts (lgl, lit);
      w = lglhts2wchs (lgl, hts);
      eow = w + hts->count;
	...
	if (tag == LRGCS) continue;
	red = blit & REDCS;
	if (skipred && red) continue;
	other = blit >> RMSHFT;
	if (abs (other) < idx) continue;
	other2 = (tag == BINCS) ? 0 : *p;
	if (other2 && abs (other2) < idx) continue;
	valother = lglval (lgl, other);
	if (valother > 0) continue;
	valother2 = (other2 ? lglval (lgl, other2) : -1);
	if (valother2 > 0) continue;
	size = 0;
	if (!valit) size++;
	if (!valother) size++;
	if (!valother2) size++;
	assert (size > 1);
	if (!valit) count[lit]++;
	if (!valother) count[other]++;
	if (!valother2) count[other2]++;
	if (size > 2) continue;
	if (!valit) bincount[lit]++;
	if (!valother) bincount[other]++;
	if (!valother2) bincount[other2]++;
      }
    }
  }

  for (glue = -1; glue < MAXGLUE; glue++) {
    if (glue >= 0 && skipred) break;
    s = glue < 0 ? &lgl->irr : &lgl->red[glue];
    for (c = s->start; c < s->top; c = p + 1) {
      p = c;
      if (*p >= NOTALIT) continue;
  ...
  pure = 0;
  for (idx = 2; !pure && !skipred && idx < lgl->nvars; idx++) {
    if (lglval (lgl, idx)) continue;
    if (lglifrozen (lgl, idx)) continue;
    for (sign = -1; !pure && sign <= 2; sign += 2) {
      lit = sign * idx;
      tmp = count[-lit];
      if (!tmp) pure = lit;
    }
  ...
  if (pure && !lgl->opts->drupligcheck.val) {
    decision = INT_MIN;
    lglprt (lgl, 2,
      "[prune-%lld-%lld-%d] pure literal %d in %d clauses",
      (LGLL) lgl->stats->prune.decs,
      (LGLL) lgl->stats->prune.calls,
      level+1,
      pure, count[pure]);
    lgl->stats->prune.pure++;
    lglunit (lgl, pure);
  } else {
    decision = 0, best = LLMAX;
    for (idx = 2; idx < lgl->nvars; idx++) {
      if (lglval (lgl, idx)) continue;
      if (lglifrozen (lgl, idx)) continue;
      pos = bincount[idx];
      neg = bincount[-idx];
      score = pos * (long) neg + pos + neg;
      if (score >= best) continue;
      decision = (pos < neg ? idx : -idx);
      best = score;
    }

    lglprt (lgl, 2,
      "[prune-%lld-%lld-%d] "
      "best decision %d with %d positive and %d negated binary occurrences",
      (LGLL) lgl->stats->prune.decs,
      (LGLL) lgl->stats->prune.calls,
      level+1,
      decision, bincount[decision], bincount[-decision]);
  }

  count -= lgl->nvars;
  bincount -= lgl->nvars;
  DEL (bincount, 2*lgl->nvars);
  DEL (count, 2*lgl->nvars);
...
static int lglprunedecidefirst (LGL * lgl) {
  int idx, sign, lit, blit, tag, red, other, other2, size, pure, tmp;
  int valit, valother, valother2, decision, glue, sat;
  const int skipred = !lglprunered (lgl);
  const int * p, * w, * eow, * c;
  int * count, bestcount;
  int64_t * sum, bestsum;
  ...
  for (idx = 2; idx < lgl->nvars; idx++) {
	...
	if (!valother2) size++;
	assert (size > 1);
	if (!valit) count[lit]++, sum[lit] += size;
	if (!valother) count[other]++, sum[other] += size;
	if (!valother2) count[other2]++, sum[other2] += size;
      }
    }
  }
  ...
  for (glue = -1; glue < MAXGLUE; glue++) {
	...
	p++;
      ...
      while ((other = *p)) {
	tmp = lglval (lgl, other);
	if (!tmp) count[other]++, sum[other] += size;
	p++;
      }
    }
  }
  ...
  pure = 0;
  if (lgl->opts->prunepure.val &&
      !lgl->opts->drupligcheck.val &&
      !skipred) {
    for (idx = 2; !pure && idx < lgl->nvars; idx++) {
      if (lglval (lgl, idx)) continue;
      if (lglifrozen (lgl, idx)) continue;
      for (sign = 1; !pure && sign >= -1; sign -= 2)
	if (!count[lit = sign * idx]) pure = -lit;
    }
  }
  ...
  if (pure) {
    decision = INT_MIN;
    lglprt (lgl, 2,
      "[prune-%lld-%lld-%d] found pure literal %d in %d clauses",
      (LGLL) lgl->stats->prune.decs,
      (LGLL) lgl->stats->prune.calls,
      lgl->level+1,
      pure, count[pure]);
    ...
    decision = 0, bestsum = 0, bestcount = INT_MAX;
    for (idx = 2; idx < lgl->nvars; idx++) {
      if (lglval (lgl, idx)) continue;
      if (lglifrozen (lgl, idx)) continue;
      for (sign = 1; !pure && sign >= -1; sign -= 2) {
	int tmpcount, update = 0;;
	int64_t tmpsum;
	lit = sign * idx;
	tmpcount = count[lit];
	tmpsum = sum[lit];
	if (tmpcount < bestcount) update = 1;
	else if (tmpcount && tmpcount == bestcount) {
	  int64_t L, R;
	  if (LLMAX / tmpcount <= bestsum) L = LLMAX;
	  else L = bestsum * tmpcount;
	  if (LLMAX / bestcount <= tmpsum) R = LLMAX;
	  else R = tmpsum * bestcount;
	  update = L < R;
	}
	if (!update) continue;
	decision = lit;
    ...
    lglprt (lgl, 2,
      "[prune-%lld-%lld-%d] "
      "best decision %d with %d occurrences and average length %.3f",
      (LGLL) lgl->stats->prune.decs,
      (LGLL) lgl->stats->prune.calls,
      lgl->level+1,
      decision, count[decision], lglavg (sum[decision], count[decision]));
  }
...
static int lglprunedecidelater (LGL * lgl) {
  int decision, shortest, size, tmp, sat, reduced, lit, glue;
  int idx, sign, blit, tag, red, other, other2;
  const int skipred = !lglprunered (lgl);
  int valit, valother, valother2;
  const int * p, * w, * eow, * c;
  HTS * hts;
  Stk * s;

  assert (lgl->level);
  assert (lgl->level < lgl->opts->prunelevel.val);

  decision = 0, shortest = INT_MAX;
  ...
  for (idx = 2; idx < lgl->nvars; idx++) {
	...
	assert (size == 2);
	tmp = 0;
	if (!valit && !lglifrozen (lgl, lit)) tmp = lit;
	if (!valother && !lglifrozen (lgl, other)) tmp = other;
	if (!valother2 && !lglifrozen (lgl, other2)) tmp = other2;
	if (!tmp || abs (tmp) >= abs (decision)) continue; 
	decision = tmp;
	shortest = 2;
      }
    }
  ...
  if (!decision)  {
	...
	p = c;
	if (*p >= NOTALIT) continue;
	reduced = sat = lit = size = 0;
	while ((other = *p)) {
	  tmp = lglval (lgl, other);
	  if (tmp > 0) sat = 1;
	  if (!tmp) {
	    if (!lglifrozen (lgl, other) &&
		(!lit || abs (other) < abs (lit)))
	      lit = other;
	    size++;
	  } else if (lglevel (lgl, other)) reduced = 1;
	  p++;
	}
	if (sat || !lit || !reduced) continue;
	assert (size > 1);
	if (size > shortest) continue;
	if (size == shortest && abs (lit) >= abs (decision)) continue;
	assert (lit);
	decision = lit;
	shortest = size;
      }
  ...
  lglprt (lgl, 2,
    "[prune-%lld-%lld-%d] "
    "best decision %d in shortest reduced clause of size %d",
    (LGLL) lgl->stats->prune.decs,
    (LGLL) lgl->stats->prune.calls,
    lgl->level+1, decision, shortest);
...
static void lglprunedecidecands (LGL * lgl, char * cands) {
  int size, reduced, sat, valit, valother, valother2, count, glue;
  int blit, tag, red, other, other2, idx, sign, lit, tmp;
  const int skipred = !lglprunered (lgl);
  const int * p, * w, * eow, * c;
  HTS * hts;
  Stk * s;
  for (idx = 2; idx < lgl->nvars; idx++)
    cands[idx] = cands[-idx] = 0;
  for (idx = 2; idx < lgl->nvars; idx++) {
    for (sign = -1; sign <= 1; sign += 2) {
      lit = sign * idx;
      valit = lglval (lgl, lit);
	...
	blit = *p;
	tag = blit & MASKCS;
	if (tag == BINCS && lgl->level) continue;
	if (tag == TRNCS || tag == LRGCS) p++;
	if (tag == LRGCS) continue;
	red = blit & REDCS;
	if (skipred && red) continue;
	...
	if (tag == TRNCS) {
	  ...
	  if (lgl->level) {
	    reduced = 0;
	    if (valit < 0 && lglevel (lgl, lit)) reduced++;
	    if (valother < 0 && lglevel (lgl, valother)) reduced++;
	    if (valother2 < 0 && lglevel (lgl, valother2)) reduced++;
	    if (!reduced) continue;
	  }
	  if (!valit && !lglifrozen (lgl, lit)) cands[lit] = 1;
	  if (!valother && !lglifrozen (lgl, other)) cands[other] = 1;
	  if (!valother2 && !lglifrozen (lgl, other2)) cands[other2] = 1;
	} else {
	  assert (!valit);
	  assert (!valother);
	  if (!lglifrozen (lgl, lit)) cands[lit] = 1;
	  if (!lglifrozen (lgl, other)) cands[other] = 1;
	}
      }
    }
  }
  for (glue = -1; glue < MAXGLUE; glue++) {
	...
	tmp = lglval (lgl, other);
	if (tmp > 0) sat++;
	if (!tmp && !lglifrozen (lgl, other)) size++;
	if (tmp < 0 && lglevel (lgl, other)) reduced++;
	p++;
      }
      if (sat || !size) continue;
      if (lgl->level && !reduced) continue;
      p = c;
      while ((other = *p)) {
	tmp = lglval (lgl, other);
	if (!tmp && !lglifrozen (lgl, other)) cands[other] = 1;
	p++;
      }
    }
  }
  ...
  for (idx = 2; idx < lgl->nvars; idx++)
    for (sign = -1; sign <= 1; sign += 2)
      if (cands[sign * idx]) count++;
  lglprt (lgl, 2,
    "[prune-%lld-%lld-%d] "
    "found %d decision candidates at level %d",
    (LGLL) lgl->stats->prune.decs,
    (LGLL) lgl->stats->prune.calls,
    lgl->level + 1, count, lgl->level);
}

static int lglprunedecidesize (LGL * lgl, char * cands, int size, int * nextsizeptr) {
  int nextsize = INT_MAX;
  *nextsizeptr = nextsize;
  return 0;
}

static int lglprunedecidephase (LGL * lgl, int idx) {
  return idx;
}

static int lglprunedecidelit (LGL * lgl) {
  int size, next, idx = 0, res;
  char * cands;
  NEW (cands, 2*lgl->nvars);
  cands += lgl->nvars;
  lglprunedecidecands (lgl, cands);
  for (size = 2;
       !idx &&
       size != INT_MAX &&
       size < lgl->opts->prunesize.val;
       size = next)
    idx = lglprunedecidesize (lgl, cands, size, &next);
  cands -= lgl->nvars;
  DEL (cands, 2*lgl->nvars);
  res = lglprunedecidephase (lgl, idx);
  return res;
}

#define lglprunedecidefirst lglprunedecidelit
#define lglprunedecidelater lglprunedecidelit

#endif

static int lglprunedecide (LGL * lgl) {
  int decision;
  if (lgl->mt) return 0;
  if (!lgl->unassigned) return 0;
  if (!lgl->opts->prune.val) return 0;
  if (lgl->opts->prunevsids.val) return 0;
  if (!lglmtstk (&lgl->assume)) return 0;
  if (lgl->level >= lgl->opts->prunelevel.val) return 0;
  lglstart (lgl, &lgl->times->prune);
  lgl->stats->prune.decs++;
  if (!lgl->level) decision = lglprunedecidefirst (lgl);
  else decision = lglprunedecidelater (lgl);
  LOG (2, "prune decision %d", decision);
  lglstop (lgl);
  return decision;
}

static void lglprunecopytrail (LGL * parent, int * map, LGL * child) {
#ifndef NLGLOG
  LGL * lgl = parent;
#endif
  const int * p;
  int size = 0;
  for (p = parent->trail.start; p < parent->trail.top; p++) {
    int plit = *p, level = lglevel (parent, plit), pidx, cidx;
    assert (lglval (parent, plit) > 0);
    if (level && !lgldecision (parent, plit)) continue;
    pidx = abs (plit);
    cidx = map[pidx];
    if (cidx) {
      int clit = plit < 0 ? -cidx : cidx;
      if (level) {
	LOG (3,
	  "adding parent decision %d negated as child external literal %d",
	  plit, -clit);
	lgladd (child, -clit);
	size++;
      } else {
	LOG (3,
	  "adding parent unit %d as external child unit %d",
	  plit, clit);
	lgladd (child, clit);
	lgladd (child, 0);
      }
    } else LOG (2, "ignoring unused parent literal %d", plit);
  }
  lgladd (child, 0);
  LOG (2, "added %d negated decisions", size);
  assert (size <= parent->level);
}

static LGL * lglprunechild (LGL * parent,
                            int level, PruneState * state) {
  LGL * child;
  assert (0 < level);
  assert (parent->level - parent->alevel == level);
  assert (level <= parent->opts->prunelevel.val);
  ...
  child->opts->prune.val = 0;
  child->opts->clim.val = parent->opts->pruneclim.val;
  child->opts->trep.val = 0;
  child->opts->druplig.val = 0;
  child->opts->drupligcheck.val = 0;
  child->opts->drupligtrace.val = 0;
  child->opts->drupligtraceorig.val = 0;
  if (child->opts->verbose.val > -1 && parent->opts->log.val < 0)
    child->opts->verbose.val--;
  if (!child->opts->plain.val) lglsetplain (child, 1);
  CLR (*state);
  state->parent = parent;
  state->child = child;
  state->map = lglnew (parent, parent->nvars * sizeof *state->map);
  lglictrav (parent, 1, 1, state, lglpruneadd);
  lglrelstk (parent, &state->clause);
  lglprt (parent, 2,
    "[prune-%lld-%lld-%d] "
    "copied %d variables in %d irredundant and %d redundant clauses",
    (LGLL) parent->stats->prune.decs, (LGLL) parent->stats->prune.calls,
    level, state->nvars, state->irr, state->red);
  lglprunecopytrail (parent, state->map, child);
  return child;
}

static int lglprunesat (LGL * parent, int level) {
  PruneState state;
  LGL * child = lglprunechild (parent, level, &state);
  int res = lglsat (child);
  ...
  lglprt (parent, 2,
    "[prune-%lld-%lld-%d] result %d in %lld conflicts and %lld decisions",
    (LGLL) parent->stats->prune.decs, (LGLL) parent->stats->prune.calls,
    level, res, (LGLL) child->stats->confs, (LGLL) child->stats->decisions);
  if (res == 10) lglpruneana (&state);
  lgldel (parent, state.map, parent->nvars * sizeof *state.map);
  lglrelease (child);
  return res;
}

static void lglprune (LGL * lgl) {
  int res, level = lgl->level - lgl->alevel;
  const char * reason;
  assert (!lgl->mt);
  assert (lgl->unassigned);
  assert (lgl->opts->prune.val);
  assert (level <= lgl->opts->prunelevel.val);
  lglstart (lgl, &lgl->times->prune);
  lgl->stats->prune.calls++;
  lgl->stats->prune.levels += level;
  lglprt (lgl, 2,
    "[prune-%lld-%lld-%d] starting pruning after %lld decisions", 
    (LGLL) lgl->stats->prune.decs, (LGLL) lgl->stats->prune.calls,
    level, lgl->stats->decisions);
  ...
  lglprt (lgl, 2,
    "[prune-%lld-%lld-%d] pruning result '%s' (%s)", 
    (LGLL) lgl->stats->prune.decs, (LGLL) lgl->stats->prune.calls,
    level, reason, res == 10 ? "successful" : "unsuccessful");
  lglupdprunelim (lgl, res == 10);
  lglstop (lgl);
  lglrep (lgl, 2, 'P');
}
...
static int lglsimplimhit (LGL * lgl, int * forcedptr) {
  int64_t n, o, d;
  int a, r, res = 0;;
  if (!lgl->opts->simplify.val) return 0;
...
static int lgloop (LGL * lgl, Lim * lim) {
  ...
  for (;;) {
    if (lglbcpsearch (lgl) && lglinprocessing (lgl)) {
      ...
      if (lglpruning (lgl)) { lglprune (lgl); continue; }
...
static void lglsetup (LGL * lgl) {
  ...
  lgl->limits->prune.inc = lgl->opts->pruneinit.val;
...
void lglstats (LGL * lgl) {
  ...
  lglprs (lgl,
    "phas: %lld flipped decision phases randomly %.0f%%",
    (LGLL) s->randphases,
    lglpcnt (s->randphases, s->decisions));
  ...
  lglprs (lgl,
    "prun: %lld decisions, %lld pure %.2f%%",
    (LGLL) s->prune.decs,
    (LGLL) s->prune.pure, lglpcnt (s->prune.pure, s->prune.decs));
  lglprs (lgl,
    "prun: %lld calls, %.1f%% per decision on level %.3f",
    (LGLL) s->prune.calls,
    lglavg (s->prune.calls, s->prune.decs),
    lglavg (s->prune.levels, s->prune.calls));
  lglprs (lgl,
    "prun: "
    "%lld sat %.0f%%, %lld unsat %.0f%%, %lld lim %.0f%%",
    (LGLL) s->prune.sat, lglpcnt (s->prune.sat, s->prune.calls),
    (LGLL) s->prune.uns, lglpcnt (s->prune.uns, s->prune.calls),
    (LGLL) s->prune.lim, lglpcnt (s->prune.lim, s->prune.calls));
  lglprs (lgl,
    "prun: %lld child conflicts, %lld decisions, %lld propagations",
    (LGLL) s->prune.child.confs,
    (LGLL) s->prune.child.decs,
    (LGLL) s->prune.child.props);
  ...
  void * state, void (*trav)(void *, int red, int lit))
{
  ...
  HTS * hts;
  REQINITNOTFORKED ();
  if (lgl->mt) { trav (state, 0, 0); return; }
  for (idx = 2; idx < lgl->nvars; idx++) {
    for (sign = -1; sign <= 1; sign += 2) {
      lit = sign * idx;
      hts = lglhts (lgl, lit);
	...
	trav (state, red, internal ? lit : lglexport (lgl, lit));
	trav (state, red, internal ? other : lglexport (lgl, other));
	if (other2) trav (state, red,
	                  internal ? other2 : lglexport (lgl, other2));
	trav (state, red, 0);
      }
    }
  }
  for (glue = -1; glue < MAXGLUE; glue++) {
    const int * start = glue < 0 ? lgl->irr.start : lgl->red[glue].start;
    const int * top = glue < 0 ? lgl->irr.top : lgl->red[glue].top;
    const int red = (glue >= 0);
    if (!redtoo && red) break;
    for (c = start; c < top; c = p + 1) {
      p = c;
      if (*p >= NOTALIT) continue;
...
typedef struct IrrTrav IrrTrav;

static void lglirrtrav (void * state, int red, int lit) {
  IrrTrav * irrtrav = state;
  assert (!red), (void) red;
  irrtrav->trav (irrtrav->state, lit);
}
...
void lglctrav (LGL * lgl, void * state, void (*trav)(void *, int)) {
  ...
  if (!lgl->mt && !lglbcp (lgl)) lglmt (lgl);
  if (!lgl->mt) lglgc (lgl);
  lglictrav (lgl, 0, 0, &irrtrav, lglirrtrav);
}
...
static void lglforkadd (void * ptr, int red, int lit) {
  assert (!red);
...
LGL * lglfork (LGL * parent) {
  ...
  child->parent = parent;
  lglcopyclonenfork (child, parent);
  if (parent->level) lglbacktrack (parent, 0);
  if (!parent->mt && !lglbcp (parent)) lglmt (parent);
  if (!parent->mt) lglgc (parent);
  lglictrav (parent, 1, 0, child, lglforkadd);
...
static int lglnextch (FILE * in, int *linenoptr) {
  int res = getc (in);
  if (res == '\n') *linenoptr += 1;
  return res;
}
...
static void lglenlbuf (LGL * lgl, Buf * b) {
  size_t new_size = b->size ? 2 * b->size : 2;
  RSZ (b->start, b->size, new_size);
  b->size = new_size;
}

static void lglpushbuf (LGL * lgl, Buf * buf, int ch) {
  if (buf->count + 1 >= buf->size) lglenlbuf (lgl, buf);
  assert (buf->count + 1 < buf->size);
  buf->start[buf->count++] = ch;
  buf->start[buf->count] = 0;
}

const char *
lglparsefile (LGL * lgl, FILE * in, int force, int * lp, int * mp) {
...
SKIP:
  ...
  if (ch == 'c') {
    ...
    while (ch != '\n') {
      ...
      while (isoptchar (ch)) {
	assert (ch != '\n');
	lglpushbuf (lgl, &buf, ch);
	ch = lglnextch (in, lp);
      }
      buf.count = 0;
      if (ch != '=') continue;
      ch = lglnextch (in, lp);
      if (ch == '-') sign = -1, ch = lglnextch (in, lp); else sign = 1;
      if (!isdigit (ch)) continue;
      val = ch - '0';
      while (isdigit (ch = lglnextch (in, lp)))
	val = 10 * val + (ch - '0');

      if (!lglhasopt (lgl, buf.start)) {
	lglwrn (lgl, "parsed invalid embedded option '--%s'\n", buf.start);
	continue;
      }
      ...
      if (!embedded++)
	lglprt (lgl, 1, ""),
	lglprt (lgl, 1, "embedded options"),
	lglprt (lgl, 1, "");
      if (!strcmp (buf.start, "verbose")) lgl->opts->verbose.val = val;
      lglprt (lgl, 1, "--%s=%d\n", buf.start, val);
      lglsetopt (lgl, buf.start, val);
    }
    goto SKIP;
  }
  DEL (buf.start, buf.size);
  if (lgl->opts->verbose.val >= 0) {
     if (embedded) lglprt (lgl, 1, "");
     else lglprt (lgl, 1,  "no embedded options");
  }
  header = section = m = n = v = l = c = 0;
  if (force) {
    if (ch == 'p')  {
      lglprt (lgl, 1, "will not read header");
      while ((ch = lglnextch (in, lp)) != '\n' && ch != EOF)
	;
    } else lglprt (lgl, 1, "skipping missing header");
    goto BODY2;
  }
  if (ch != 'p') return "missing 'p ...' header";
  if (lglnextch (in, lp) != ' ')
    return "invalid header: expected ' ' after 'p'";
  while ((ch = lglnextch (in, lp)) == ' ')
    ;
  if (ch != 'c') return "invalid header: expected 'c' after ' '";
  if (lglnextch (in, lp) != 'n')
    return "invalid header: expected 'n' after 'c'";
  if (lglnextch (in, lp) != 'f')
    return "invalid header: expected 'f' after 'n'";
  if (lglnextch (in, lp) != ' ')
    return "invalid header: expected ' ' after 'f'";
  while ((ch = lglnextch (in, lp)) == ' ')
    ;
  ...
  while (ch == ' ')
    ch = lglnextch (in, lp);
  if (ch == '\r') ch = lglnextch (in, lp);
  if (ch != '\n') return "invalid header: expected new line after header";
  lglprt (lgl, 1, "found 'p cnf %d %d' header", m, n);
  *mp = m;
  header = 1;
BODY:
  ch = lglnextch (in, lp);
...
DONE:
    lglprt (lgl, 1,
      "read %d variables, %d clauses, %d literals in %.2f seconds", 
      v, c, l, lglsec (lgl));
    return 0;
  }
  if (ch == '-') {
  ...
  if (section) {
    ...
    lgladd (lgl, lit);
    if (!lit && force && c == n) goto DONE;
  }
  goto BODY;
}

const char *
lglparsepath (LGL * lgl, const char * path, int force, int * lp, int * mp)
{
  const char * res;
  size_t len;
  FILE * in;
  int clin;
  char * tmp;
  lglprt (lgl, 1, "reading DIMACS file '%s'", path);
  len = strlen (path);
  if (len >= 3 && !strcmp (path + len - 3, ".gz")) {
    lglprt (lgl, 1, "piping '%s' through 'gzip'", path);
    tmp = malloc (len + 20);
    sprintf (tmp, "gzip -c -d %s", path);
    in = popen (tmp, "r");
    if (in) clin = 2;
    free (tmp);
  } else if (len >= 3 && !strcmp (path + len - 3, ".xz")) {
    lglprt (lgl, 1, "piping '%s' through 'xz'", path);
    tmp = malloc (len + 20);
    sprintf (tmp, "xz -c -d  %s", path);
    in = popen (tmp, "r");
    if (in) clin = 2;
    free (tmp);
  } else if (len >= 4 && !strcmp (path + len - 4, ".bz2")) {
    lglprt (lgl, 1, "piping '%s' through 'bzip2'", path);
    tmp = malloc (len + 20);
    sprintf (tmp, "bzip2 -c -d %s", path);
    in = popen (tmp, "r");
    if (in) clin = 2;
    free (tmp);
  } else if (len >= 4 && !strcmp (path + len - 4, ".zip")) {
    lglprt (lgl, 1, "piping '%s' through 'unzip'", path);
    tmp = malloc (len + 20);
    sprintf (tmp, "unzip -p %s", path);
    in = popen (tmp, "r");
    if (in) clin = 2;
    free (tmp);
  } else if (len >= 3 && !strcmp (path + len - 3, ".7z")) {
    lglprt (lgl, 1, "piping '%s' through '7z'", path);
    tmp = malloc (len + 40);
    sprintf (tmp, "7z x -so %s 2>/dev/null", path);
    in = popen (tmp, "r");
    if (in) clin = 2;
    free (tmp);
  } else {
  ...
  *lp = 0;
  if (!in) return "can not read";
  res = lglparsefile (lgl, in, force, lp, mp);
  if (clin == 1) fclose (in);
  if (clin == 2) pclose (in);
  return res;
}
